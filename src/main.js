...
export default async ({ req, res, log, error }) => {
  ...
  try {
    const { action, amount, currency, id, sessionId } = JSON.parse(req.body || '{}');
    ...
    if (action === 'create_session') {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [{
          price_data: { currency, product_data: { name: 'My Product' }, unit_amount: amount },
          quantity: 1
        }],
        mode: 'payment',
        success_url: `http://localhost:64426/payment-success/${id}?session_id=${session.id}`,
        cancel_url: `http://localhost:64426/payment-cancel/${id}?session_id=${session.id}`,
      });
